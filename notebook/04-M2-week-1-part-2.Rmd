...
### pratical exercise
@wickham2017r shows some practical plotting examples using the _tiddyverse_, a collection of very usefull packages for data manipulation. If you want to know more about packages you can head to the correspondig seccion. 
...
## R Packages
One of the advantages of open source is that the community can creates packages which extend the functionality of the core R implementation. There exist many ways to access this packages, the larger collection is provided via the CRANN repository but Git based packages are also supported. In order to use this packages it is necesary not only to install the package, but also load it into the R.  
