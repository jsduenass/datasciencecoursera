...
## JSON
JavScript Objetc Notation is way of storing data in a structured manner, used extensibily in API's.  transform datas sets and turn it into JSON format
...
head(DT,3)
```
"Expressions" add new columns,  if  a data table is assign to a new varaible they become intertwined, so any change in one will afect the other, it is better to create copy through the ```copy``` function.
```setkey```
